...
let store = null, sessionId = null, setupStill = null;
...
function render(tracks) {
  ...
  if (settings.privacy === "debug") {
    ...
    for (const tr of tracks || []) {
      ...
      dctx.fillStyle = "rgba(110,231,155,.9)"; dctx.font = "12px system-ui";
      dctx.fillText("#" + tr.id, s.x + 8, s.y);
    }
  }
...
for (const radio of document.querySelectorAll('input[name="privacy"]')) {
  radio.addEventListener("change", (e) => {
    ...
    $("privacyNote").textContent = settings.privacy === "normal"
      ? "Normal mode stores observations only. No stills or footage are saved."
      : settings.privacy === "setup"
        ? "Audit mode also stores ONE setup still documenting how the view is configured."
        : "Debug mode shows the tracker overlay. No event stills or footage are saved.";
    $("btnSetupStill").disabled = settings.privacy === "normal";
  });
}

// Setup still
$("btnSetupStill").addEventListener("click", () => {
  if (!cam.videoWidth) { statusLine.textContent = "start the camera first."; return; }
  const c = document.createElement("canvas");
  c.width = cam.videoWidth; c.height = cam.videoHeight;
  c.getContext("2d").drawImage(cam, 0, 0);
  c.toBlob((blob) => {
    setupStill = { blob, filename: `setup-${new Date().toISOString().slice(0, 10)}.jpg` };
    $("btnSetupStillDownload").disabled = false;
    statusLine.textContent = "Setup still captured (kept locally).";
  }, "image/jpeg", 0.8);
});
...
$("btnSetupStillDownload").addEventListener("click", () => {
  if (!setupStill) return;
  const a = document.createElement("a");
  a.href = URL.createObjectURL(setupStill.blob); a.download = setupStill.filename; a.click();
  URL.revokeObjectURL(a.href);
});
